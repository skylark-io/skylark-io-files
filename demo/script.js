...
function initPage($,files) {
    ...
    window.fs = files.fs;
    window.path = require("skylark-langx-paths");
    ...
    // --------------------------------------------------------------
    var term = originalJquery('.term').terminal((command) => {
        var cmd = originalJquery.terminal.parse_command(command);
